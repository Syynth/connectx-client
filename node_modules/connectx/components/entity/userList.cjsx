...
{AutoStoreListener, StoreListener, DebugProps, ImageUrl, EnsureFetch} = require 'connectx/mixins'

UserFetch = EnsureFetch UserStore
Listener = AutoStoreListener UserStore

mixins = [DebugProps, ImageUrl, AutoStoreListener(UserStore), EnsureFetch(UserStore)]
...
UserAdminView = React.createClass
  ...
  render: ->
    <li className= "grid-100 bg-lv-1 bs-lv-1 brad-5-px ptb-3 ">
      <Link to="user" params={id: @props.id}>
        <h3 className ="grid-80 c-green lh-4 brad-5-px bs-lv-1 va-c ">Name: {@props.name} (Admin)</h3>
        <figure className ="grid-20 bs-lv-1 brad-5-px bg-white of-h">
          <img className ="grid-100 brad-5-px" src={@getProfilePhoto @props} />
        </figure>
      </Link>
...
UserRequestView = React.createClass
  ...
  render: ->
    <li className= "grid-100 bg-lv-1 bs-lv-1 brad-5-px ptb-3 ">
      <Link to="user" params={id: @props.id}>
        <h3 className ="grid-80 c-green lh-4 brad-5-px bs-lv-1 va-c ">Name: {@props.name} (Request)</h3>
        <figure className ="grid-20 bs-lv-1 brad-5-px bg-white of-h">
          <img className ="grid-100 brad-5-px" src={@getProfilePhoto @props} />
        </figure>
      </Link>
