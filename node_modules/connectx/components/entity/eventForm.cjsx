...
TimePicker = require 'connectx/components/timePicker'
...
EventForm = React.createClass
  ...
  cancelForm: (ev) ->
    ...
    ev.stopPropagation()
    @props.onRequestHide()
  updateStartTime: (hour, min, am) ->
    @onFormChange {target: {name: 'start'}, value: {hour, min, am}}
  updateEndTime: (hour, min, am) ->
    @onFormChange {target: {name: 'end'}, value: {hour, min, am}}
  updateDate: (date) ->
    @onFormChange {target: {name: 'date'}, value: date}
  createEvent: (ev) ->
    ev.preventDefault()
    ev.stopPropagation()
    if @validate()
            ...
            <div className="col3 combo modal_section clearfix">
              ...
              <div className="modal_section time clearfix">
                <section className="col3">
                  ...
                  <DatePickerInput onChangeDate={@updateDate}  /> {### haha woot it works ###}
                </section>
                <section className="col3">
                  <label htmlFor="s_time">Start Time</label>
                  <TimePicker onChange={@updateStartTime} />
                </section>
                <section className="col3">
                  <label htmlFor="e_time">End Time</label>
                  <TimePicker onChange={@updateEndTime} />
                </section>
              </div>
            </div>
