...
module.exports = React.createClass
  ...
  updatePreview: (ev) ->
    ...
    if (el.files and el.files[0])
      ...
      reader.onload = (e) =>
        @setState imgSrc: e.target.result, file: file
      reader.readAsDataURL file
  submit: (ev) ->
    ev.stopPropagation()
    ev.preventDefault()
    FileActionCreators.uploadFileToEntity @props.current, @state.file, @props.fieldName
    @setState imgSrc: "", file: null
  render: ->
    id = guid.create()
    btnStyle =
      if @state.imgSrc isnt ""
        display: 'block'
      else
        display: 'none'
    ...
    <div style={@props.style}>
      <figure className="preview-img-wrapper">
        {
          if @state.imgSrc
            ...
            <img ref="preview" className="preview-file" src={dataUrl} />
        }
      </figure>
      ...
      <button onClick={@submit} style={btnStyle} className="submit-file button green">Upload</button>
    </div>
