...
migrateCommentToServerId = (oldId, newId, postId, self) ->
  post = p.get postId
  index = post.comments.indexOf oldId
  if index > 0
    post[index] = newId
  self.write 'postCache', p.data
  self.emitChange()
...
addComments = (data, self) ->
  post = p.get data.postId
  post.comments = post.comments || []
  post.comments.push data.clientId
  self.write 'postCache', p.data
  self.emitChange()
...
class PostStore extends BaseStore
  ...
  ,
    ...
    fn: (payload) ->
      migrateCommentToServerId payload.action.clientId, payload.action.serverId, payload.action.postId, @
  ,
    source: SourceType.Server
