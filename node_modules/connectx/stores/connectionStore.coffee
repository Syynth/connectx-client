...
class ConnectionStore extends BaseStore
  ...
  dependents: [GroupStore, UserStore, CourseStore, EventStore]
  ...
  # deletes an edge connection two nodes
  deleteEdge: (node, edgeNode) ->
    cns = @_get node.id
    cns = cns.filter (cn) -> cn.id isnt edgeNode.id
    @set node.id, cns

  # determine whether there is any connection from origin to target
  hasEdge: (to, from) -> -1 isnt _.findIndex (@_get to.id), id: from.id
  ...
  ,
    ...
    fn: (action) -> @recordEdge action.from,
      id: action.to.id
      type: action.to.type
      pending: !@hasEdge(action.to, action.from)
  ,
    action: ActionType.ConnectionDeleted
    fn: (action) -> @deleteEdge action.from, action.to
  ,
    action: ActionType.ConnectionQuery
    fn: (action) ->
      action.entities.map (node) => @recordNode node
  ]
